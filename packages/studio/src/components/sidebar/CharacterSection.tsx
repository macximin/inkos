...
const TIER_BADGE: Record<RoleRef["tier"], { zh: string; ko: string; en: string; color: string }> = {
  major: { zh: "主要", ko: "주요", en: "Major", color: "bg-amber-500/15 text-amber-600 dark:text-amber-400" },
  minor: { zh: "次要", ko: "보조", en: "Minor", color: "bg-blue-500/15 text-blue-600 dark:text-blue-400" },
};
    ...
    <div className="rounded-lg bg-secondary/30 overflow-hidden">
      ...
      {expanded && (
        <div className="px-2.5 pb-2.5 space-y-1">
          {tags && (
            <p className="text-[14px] leading-6 text-muted-foreground"><span className="text-muted-foreground/60">{tr("标签", "Tags", "태그")}</span> {tags}</p>
          )}
          {current && (
            <p className="text-[14px] leading-6 text-muted-foreground"><span className="text-muted-foreground/60">{tr("当前", "Current", "현재")}</span> {current}</p>
          )}
...
export function CharacterSection({ bookId }: CharacterSectionProps) {
  ...
  return (
    <SidebarCard title={tr("角色", "Characters", "인물")}>
