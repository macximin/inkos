...
const AGENT_LABELS: Record<string, readonly [string, string, string]> = {
  architect: ["建书", "Create book", "책 만들기"],
  writer: ["写作", "Write", "집필"],
  auditor: ["审计", "Audit", "검수"],
  reviser: ["修订", "Revise", "수정"],
  exporter: ["导出", "Export", "내보내기"],
};

const TOOL_LABELS: Record<string, readonly [string, string, string]> = {
  read: ["读取文件", "Read file", "파일 읽기"],
  edit: ["编辑文件", "Edit file", "파일 편집"],
  grep: ["搜索", "Search", "검색"],
  ls: ["列目录", "List directory", "목록 보기"],
  ...
  script_create: ["剧本创作", "Create script", "대본 제작"],
  storyboard_create: ["分镜创作", "Create storyboard", "스토리보드 제작"],
  interactive_film_create: ["互动影游", "Interactive film", "인터랙티브 영상"],
